...
- ✅ 2026-08-14 G9 內容路由探測(成對,各 4 輪):**兩臂零差異** → krepo 決策樹不上收,留在 krepo。
  副產物比原目標值錢——測出「已知缺口」節名歧義(見技術債同日條),且候選規則的適配句反而成為
  繞過判準的逃生口。996 PASS。
...
- ✅ 2026-08-14 dossier 治理三件事(起點:使用者反映「一直在處理 dossier flag、很花時間」)。
  ...
  `docs/dead-ends.md`、本節只留「會擋住你的那一句」(5123→3006 bytes,模式照 known-hazards.md);
