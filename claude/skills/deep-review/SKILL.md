...
**修復原則**：主 agent 在修復階段依照 subagent 的修復計畫執行，不跳過任何等級的問題。修復後必須 commit 再進入下一輪審查。
...
- 有 commit → working tree clean，Step 1 自動使用 `git diff main...HEAD` 審查完整 branch 狀態
...
**銜接檢查**：若 working tree 有大量變更且 `git log main..HEAD` 已有 commit，提醒使用者是否忘記 commit 上一輪修復。這違反迭代紀律，應先 commit 再繼續 review。
...
**通過標準**（全部滿足）：零嚴重、零中等、零建議、整體性通過、跨檔案契約一致、跨 repo 一致性通過（多 repo 時）。

嚴重度用於**修復排序**（先嚴重 → 中等 → 建議），不用於篩選——所有等級的問題都必須處理。
...
- **區分等級** — 等級決定修復順序，不決定是否修復；所有等級都必須處理
