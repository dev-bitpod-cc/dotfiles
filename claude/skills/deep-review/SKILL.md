...
- 最終通過後，主 agent 負責 squash——範圍為所有 review fix commits（base..HEAD），commit message 採原始功能變更的語意（不是 `fix: review fixes`），格式遵循專案 Conventional Commits 慣例，並附環境指定的 `Co-Authored-By` trailer（以 runtime system prompt 的 Git 區塊為權威，勿在 skill 寫死 model 名稱/版本）
