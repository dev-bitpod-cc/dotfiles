...
**上限**：3 輪 codex 審查、2 輪修復。到此階段 code 已通過主 agent 完整審查，剩餘問題應快速收斂。若第 3 輪仍有 true positive blocking findings → 停止，輸出 codex 終止報告交使用者。
...
**Commit range 更新**：每輪修復 commit 後，下一輪 codex 審查的 commit range 更新為涵蓋最新修復的完整範圍。base 端固定錨在「本次審查起點」（Step 1 判定的 base commit hash），不要隨修復推進而前移——否則只會審到最後一個 commit、漏掉前面的修復。若 base 已被 push（`origin/main..HEAD` 為空），在第一輪 codex 審查前先記下當時的 base hash，後續各輪一律用 `<記下的 base hash>..HEAD`，不要退化成 `HEAD~1..HEAD`。
