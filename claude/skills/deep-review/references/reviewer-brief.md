...
**本節掃的是命中點軸**——這條規則在**既有 code 的其他地方**還有沒有犯同樣的錯。另有兩條屬 fixer 職責，見 `../SKILL.md`「修復原則」：**輸入空間軸**（修復對該規則的**所有輸入**是否成立）與**相依軸**（誰的正確性依賴被改的東西——依關係找，不是找重複出現的字）。三軸同名不同軸，**no axis is evidence for another**。
