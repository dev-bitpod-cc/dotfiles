...
- **死路的收斂手段是分層,不是豁免。**「不刪」管的是**結論句**,不是整節不動——推導過程、
  實測數字、eval 編號、事故細節外移到 `docs/dead-ends.md`(先例見該檔「分工」),STATUS.md
  只留**會擋住你的那一句**。這屬收斂順序的「②歸檔留指標」,不是它的例外。
  **實證缺這一句的後果**:dotfiles 2026-08-18 兩次貼門檻收斂(24351→20406、24285→21577)
  死路節 6065 bytes **一 byte 未動**,全由決策節吸收;而它自 08-14 分層後 2964→7003
  只增不減。原因是本節唯一指名死路的話是「不刪」,分層出口只寫在該 repo 自己的 STATUS.md
  檔頭、規範層讀不到 → 保守的處置就是跳過。⚠️ 條目 flag **不掃死路節**
  (`ship-state.sh` 的節名錨定只含決策/里程碑),故該節無單條上限守著,全靠這一條。
- **分層後,結論與證據的生命週期綁在一起。** 死路只有兩種離場方式,處置不同:
  - **翻案**(該條「結論會翻的條件」成立、那條路現在走得通):**比照被推翻的決策**——
    STATUS.md 那條保留原文加失效標記、指向現行做法,**不刪**;`docs/dead-ends.md` 對應節
    **一併保留**。當初為什麼判死、以及**什麼條件讓它翻過來**,正是防止第三次來回的東西。
  - **不再適用**(整個主題消失,如該功能已移除):兩邊**同一次 commit 一起移除**。
  - **NEVER remove one side alone.** 只刪 STATUS.md 那條 → 證據節變節級孤兒(內容還在
    git 裡但走不到);只刪證據節 → 指標斷掉。前者靜默,後者不靜默,**兩者都由機械層兜底**:
    正向斷指標與反向節級孤兒都在 `tests/xref-gate.py`(**僅 dotfiles 有此 gate**,其他 repo
    只有這條規範)。2026-08-19 首次反向掃描:12 節中 5 節孤兒。
