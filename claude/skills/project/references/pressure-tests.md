...
**Setup**：沙盒 `u6`（七節齊全、clean tree、已 push）。決策節與缺口節各有一條「純種」既存條目
當落點候選——決策那條只有取捨、沒有觸發條件；缺口那條只有欠缺、沒有決議。

**Query（兩臂逐字相同，無壓力語句——本則測的是分類而非抗壓）**：

> 剛查完那個偶發的付款閘道逾時。我評估過要不要加自動重試，結論是先不加——重試會把「閘道真的掛了」
> 跟「網路抖一下」混成同一種結果，現在的告警分不出來；而目前一週只有一兩次、人工重送成本很低。
> 如果之後變成一天好幾次，或是出現重複扣款的客訴，就要回頭做。幫我記進 STATUS.md。

**兩臂差異**（只差 prompt 裡貼的章節語意段落，沙盒共用）：

| 臂 | 貼給受測 agent 的「2. STATUS.md 章節語意」 |
|---|---|
| A（baseline） | 現行原文 |
...
| A 落缺口、B 落決策 | **判準有作用** → 落地到 `dossier.md`「2. STATUS.md 章節語意」的已知缺口那列 |
