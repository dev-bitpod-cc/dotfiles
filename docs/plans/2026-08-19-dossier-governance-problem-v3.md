...
- 日期：2026-08-19（**v3**——取代 v2；v1／v2 依 `AGENTS.md:41` write-once 凍結不改）
- 性質：**問題陳述與選項評估**，不是實作計畫
...
| 3 | **問題陳述 v2（`516ab09` 發布）又被 `6b563ce` 就地增修** | 本次：v2 已還原成 `516ab09`，修正落成本檔 v3 |
...
| 項 | 設計 |
|---|---|
| 鍵 | `(session_id, repo, path, corpus revision)` |
| 生命週期 | 命中後放行該路徑，直到 corpus revision 變動 |
| 重置條件 | corpus 檔案有新 commit（revision 變）／session 結束 |
| 存放 | session-scoped 暫存，**不進 git**（衍生狀態） |

**沒有這個 latch，整個機制不能實作**——這是 §5.2 目前最大的未完成處。
...
⇒ **deny-first 只能承諾 `Write`／`Edit` 這條防線，不能承諾「第一次讀取之前必然召回」。** v2 只提了 Bash 寫檔旁路，漏了 Bash／Grep 讀取與 `@file`。
...
⇒ 動手前先建 eval：**給定一個必須依賴某條已歸檔決策／死路才能做對的任務**，量 baseline vs hook 版本的**召回率、誤召回率、額外 turns、token、延遲**。沒有這組數字，「hook 有沒有用」只是推論。
